  ...
  for (method in model_methods) {
    ...
    if (method == "linear") {
      ...
      if (binary_Y) {
        ...
        lambda_Y         <- cv_lasso_Y_model$lambda.min
        lasso_model_Y    <- glmnet(x = data.matrix(X_dataset), y = data.matrix(Y_column), alpha=1, lambda=lambda_Y)
      }
      ...
      Y_vars_selected <- vars_selected[Y_vars_selected != "(Intercept)"]
